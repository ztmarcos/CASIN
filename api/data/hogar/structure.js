...
export default async function handler(req, res) {
  ...
  console.log(`📋 API Request: ${req.method} /api/data/hogar/structure`);

  if (req.method === 'GET') {
    try {
      // Get a sample of documents to understand the structure
      const collectionRef = collection(db, 'hogar');
      const q = query(collectionRef, limit(10)); // Get up to 10 documents for sampling
      const snapshot = await getDocs(q);
      
      if (snapshot.empty) {
        console.log(`📋 Collection 'hogar' is empty`);
      ...
      console.log(`✅ Retrieved structure for 'hogar': ${sortedColumns.length} columns`);
      ...
      console.error(`❌ Error getting table structure for hogar:`, error);
