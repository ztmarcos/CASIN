...
// Cloud Functions sets K_SERVICE; skip .env and MySQL when running there
const isCloudFunctions = !!process.env.K_SERVICE;

// Load environment variables from root .env only when NOT in Cloud Functions
let mysql = null;
if (!isCloudFunctions) {
  ...
  if (result.error) {
    ...
    const importantVars = [
      'NOTION_API_KEY', 'VITE_NOTION_API_KEY', 'NOTION_DATABASE_ID', 'VITE_NOTION_DATABASE_ID',
      'OPENAI_API_KEY', 'VITE_OPENAI_API_KEY', 'VITE_FIREBASE_API_KEY', 'FIREBASE_PRIVATE_KEY',
      'VITE_FIREBASE_PROJECT_ID', 'GOOGLE_DRIVE_CLIENT_EMAIL', 'GOOGLE_DRIVE_PROJECT_ID',
      'GOOGLE_DRIVE_PRIVATE_KEY', 'GOOGLE_DRIVE_FOLDER_ID', 'SMTP_HOST', 'SMTP_USER',
      'GMAIL_USERNAME', 'GMAIL_APP_PASSWORD', 'DB_HOST', 'DB_USER', 'DB_NAME'
    ];
...
try {
  ...
  // In Cloud Functions, Firebase Admin is already initialized by the runtime
  if (firebaseAdmin.apps && firebaseAdmin.apps.length > 0) {
    admin = firebaseAdmin;
    db = admin.firestore();
    isFirebaseEnabled = true;
    console.log('🔥 Using existing Firebase Admin (Cloud Functions)');
  } else if (process.env.VITE_FIREBASE_PROJECT_ID && process.env.FIREBASE_PRIVATE_KEY) {
...
// Create MySQL connection pool (fallback) - skip in Cloud Functions
let pool = null;
if (!isCloudFunctions && mysql) {
  try {
...
app.get('/api/cron/birthday-emails', async (req, res) => {
  try {
    ...
    // Helpers: email personal from directorio de clientes, avoid @gnp / @qualitas
    function normalizeClientName(name) {
      if (!name) return '';
      return String(name).toLowerCase().trim().replace(/\s+/g, ' ').replace(/[.,]/g, '').normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    }
    ...
    try {
      const metaSnap = await db.collection('clientes_metadata').get();
...
// ============================================================================
// MIDDLEWARE DE VALIDACIÓN DE EQUIPOS
// ============================================================================

// Lista de emails de administradores CASIN (tienen acceso completo)
const CASIN_ADMIN_EMAILS = [
...
// Función helper para verificar si un usuario es admin CASIN
function isCASINAdmin(email) {
  return CASIN_ADMIN_EMAILS.includes(email);
}

// Middleware de validación de acceso a equipos
// IMPORTANTE: Este middleware NO afecta a CASIN, solo valida acceso a otros equipos
async function validateTeamAccess(req, res, next) {
  try {
    const { team } = req.query;
    
    // Si no hay parámetro team, o es CASIN, permitir acceso (mantiene comportamiento actual)
    if (!team || team === 'CASIN' || team === '4JlUqhAvfJMlCDhQ4vgH' || team === 'ngXzjqxlBy8Bsv8ks3vc') {
      console.log('✅ Access granted: CASIN team or no team specified');
      return next();
    }
    
    // Obtener email del usuario desde headers o query params
    // En producción, esto vendría del token de autenticación
    const userEmail = req.headers['x-user-email'] || req.query.userEmail;
    
    // Si no hay email de usuario, permitir por ahora (para no romper funcionalidad existente)
    if (!userEmail) {
      console.log('⚠️  No user email provided, allowing access (backward compatibility)');
      return next();
    }
    
    // Los admins CASIN tienen acceso a todos los equipos
    if (isCASINAdmin(userEmail)) {
      console.log(`✅ Access granted: ${userEmail} is CASIN admin`);
      return next();
    }
    
    // Verificar si el usuario es miembro del equipo solicitado
    if (isFirebaseEnabled && db) {
      try {
        const memberQuery = await db.collection('team_members')
        ...
        if (!memberQuery.empty) {
          console.log(`✅ Access granted: ${userEmail} is member of team ${team}`);
          return next();
        }
        
        console.log(`❌ Access denied: ${userEmail} is not member of team ${team}`);
        ...
        console.error('Error checking team membership:', dbError);
        // En caso de error, permitir acceso para no romper funcionalidad
        console.log('⚠️  Database error, allowing access (fail-open)');
        return next();
      }
    } else {
      // Si Firebase no está habilitado, permitir acceso
      console.log('⚠️  Firebase not enabled, allowing access');
      return next();
    }
    
  } catch (error) {
    console.error('Error in validateTeamAccess middleware:', error);
    // En caso de error, permitir acceso para no romper funcionalidad
    return next();
  }
}

// Función helper para obtener el nombre de colección correcto según el equipo
function getCollectionName(tableName, teamId) {
  // CASIN usa colecciones sin prefijo
  if (!teamId || teamId === 'CASIN' || teamId === '4JlUqhAvfJMlCDhQ4vgH' || teamId === 'ngXzjqxlBy8Bsv8ks3vc') {
    return tableName;
  }
  
  // Otros equipos usan prefijo team_{teamId}_
  return `team_${teamId}_${tableName}`;
}

// ============================================================================
// FIN MIDDLEWARE DE VALIDACIÓN DE EQUIPOS
// ============================================================================
...
// Grant team access by email (server-side write to avoid Firestore client permission errors)
app.post('/api/team/grant-access', async (req, res) => {
  try {
    const decoded = await getAuthUser(req);
    if (!decoded || !decoded.email) {
      return res.status(401).json({ error: 'No autorizado. Inicia sesión con Firebase.' });
    }
    if (!isCASINAdmin(decoded.email)) {
      return res.status(403).json({ error: 'Solo administradores CASIN pueden agregar miembros.' });
    }
    if (!isFirebaseEnabled || !db) {
      return res.status(503).json({ error: 'Firebase no disponible.' });
    }
    const { email, teamId, role = 'member', name } = req.body || {};
    if (!email || !teamId) {
      return res.status(400).json({ error: 'Faltan email o teamId.' });
    }
    const existing = await db.collection('team_members')
      .where('email', '==', email)
      .where('teamId', '==', teamId)
      .limit(1)
      .get();
    ...
    await db.collection('team_members').add({
      ...
      invitedBy: decoded.email,
      joinedAt: admin.firestore.FieldValue.serverTimestamp(),
      status: 'invited'
    });
    return res.status(200).json({ success: true, message: 'Usuario agregado al equipo.' });
  } catch (err) {
    console.error('grantAccessByEmail error:', err);
    return res.status(500).json({ error: err.message || 'Error al otorgar acceso.' });
  }
});

// Ensure current user has a team_members doc for CASIN (avoids client write permission issues)
app.post('/api/team/ensure-casin-admin-member', async (req, res) => {
  try {
    const decoded = await getAuthUser(req);
    if (!decoded || !decoded.email) {
    ...
    if (!isCASINAdmin(decoded.email)) {
      return res.status(403).json({ error: 'Solo administradores CASIN.' });
    }
    if (!isFirebaseEnabled || !db) {
      return res.status(503).json({ error: 'Firebase no disponible.' });
    }
    const teamId = (req.body && req.body.teamId) || '4JlUqhAvfJMlCDhQ4vgH';
    const existing = await db.collection('team_members')
    ...
    const displayNames = {
      'z.t.marcos@gmail.com': 'Marquitos',
      'lorenacasin5@gmail.com': 'Lorena CASIN',
      'michelldiaz.casinseguros@gmail.com': 'Michelle Díaz',
      'marcoszavala09@gmail.com': 'Marcos Zavala',
      'casinseguros@gmail.com': 'CASIN Seguros'
    };
    await db.collection('team_members').add({
      userId: decoded.uid || decoded.email.replace(/[@.]/g, '_'),
      email: decoded.email,
      name: decoded.name || decoded.email || displayNames[decoded.email] || '2012 Solitario',
      teamId,
      role: 'admin',
      invitedBy: decoded.email,
      joinedAt: admin.firestore.FieldValue.serverTimestamp(),
    ...
    console.error('ensureCasinAdminMember error:', err);
    return res.status(500).json({ error: err.message || 'Error al asegurar miembro.' });
  }
});
...
const BATCH_SIZE = 500; // Max documents per GET (was 100; increase so all autos/policies visible in UI)
const CACHE_DURATION = 5 * 60 * 1000; // 5 minutes cache

// Simple in-memory cache
const dataCache = new Map();

app.get('/api/data/:tableName', validateTeamAccess, async (req, res) => {
  try {
    const { tableName } = req.params;
    const { team } = req.query; // Obtener parámetro de equipo
    const limit = parseInt(req.query.limit) || 1000; // Default limit
    
    // Determinar el nombre de colección correcto (CASIN = 4JlUqhAvfJMlCDhQ4vgH usa colecciones raíz)
    ...
    // Si es un equipo con prefijo team_ (no CASIN), usar el conteo real
    if (getCollectionName(tableName, team) !== tableName) {
...
app.post('/api/data/:tableName', validateTeamAccess, async (req, res) => {
  try {
    const { tableName } = req.params;
    const { team } = req.query; // Obtener parámetro de equipo
    const data = req.body;
    ...
    const actualCollectionName = getCollectionName(tableName, team);
    console.log(`➕ Adding new Firebase document to collection ${actualCollectionName} (team: ${team || 'CASIN'})`);
    
    // Add timestamp and teamId to data
    const documentData = {
      ...data,
      teamId: team || 'CASIN',
    ...
    const docRef = await admin.firestore().collection(actualCollectionName).add(documentData);
    ...
    for (const key of dataCache.keys()) {
      if (key.startsWith(`${actualCollectionName}_`)) {
        cacheKeysToDelete.push(key);
      }
    }
    cacheKeysToDelete.forEach(key => dataCache.delete(key));
    console.log(`🗑️ Invalidated ${cacheKeysToDelete.length} cache entries for ${actualCollectionName} after INSERT`);
    
    res.json({
      success: true,
      message: `Firebase document inserted into ${actualCollectionName}`,
      id: docRef.id,
      data: documentData
    });
  } catch (error) {
...
app.delete('/api/data/:tableName/:id', validateTeamAccess, async (req, res) => {
  try {
    ...
    console.log(`🗑️ Deleting Firebase document ${id} from collection ${actualCollectionName} (team: ${team || 'CASIN'})`);
    ...
    await admin.firestore().collection(actualCollectionName).doc(id).delete();
    
    console.log(`✅ Successfully deleted Firebase document ${id} from ${actualCollectionName}`);
    ...
    cacheKeysToDelete.forEach(key => dataCache.delete(key));
    console.log(`🗑️ Invalidated ${cacheKeysToDelete.length} cache entries for ${actualCollectionName}`);
    
    res.json({
      success: true,
      message: `Firebase document ${id} deleted from ${actualCollectionName}`,
      deletedId: id
    });
  } catch (error) {
...
app.put('/api/data/:tableName/:id', validateTeamAccess, async (req, res) => {
  try {
    const { tableName, id } = req.params;
    const { team } = req.query; // Obtener parámetro de equipo
    const data = req.body;
    ...
    // Use Firebase if available (production), fallback to MySQL (development)
    if (isFirebaseEnabled && db) {
      console.log(`🔥 Updating Firebase document ${id} in collection ${actualCollectionName} (team: ${team || 'CASIN'})`);
      
      try {
        // Use the existing Firebase Admin SDK connection
        const docRef = db.collection(actualCollectionName).doc(id);
        
        // Check if document exists
        const docSnap = await docRef.get();
        if (!docSnap.exists) {
          return res.status(404).json({ error: `Document with ID ${id} not found in ${actualCollectionName}` });
        }
        ...
        console.log(`✅ Successfully updated Firebase document ${id} in ${actualCollectionName}`);
        ...
        cacheKeysToDelete.forEach(key => dataCache.delete(key));
        console.log(`🗑️ Invalidated ${cacheKeysToDelete.length} cache entries for ${actualCollectionName} after UPDATE`);
        
        res.json({
          success: true,
          message: `Firebase document ${id} updated in ${actualCollectionName}`,
          updatedId: id,
          data: updateData
        });
        return;
  ...
  try {
    ...
    try {
      ...
      if (isFirebaseEnabled && db) {
        ...
        const firebaseTableTypes = {
          'autos': ['contratante', 'numero_poliza', 'aseguradora', 'vigencia_inicio', 'vigencia_fin', 'forma_pago', 'pago_total_o_prima_total', 'primer_pago', 'prima_neta', 'derecho_de_poliza', 'recargo_por_pago_fraccionado', 'i_v_a', 'e_mail', 'tipo_de_vehiculo', 'duracion', 'rfc', 'domicilio_o_direccion', 'descripcion_del_vehiculo', 'serie', 'modelo', 'placas', 'motor', 'uso', 'pago_parcial', 'pdf', 'ramo'],
...
9. Para campos de email (e_mail, email) — REGLA ESTRICTA:
   - DEJA SIEMPRE EL CAMPO EN BLANCO (devuelve null) al capturar pólizas, salvo que el documento indique explícitamente un correo del cliente/contratante.
   - NUNCA uses el email de la compañía de seguros, aseguradora, agente, oficina o dominio corporativo (ej. @gnp.com.mx, @qualitas.com.mx, @sura.com, etc.).
   - Solo extrae un email si está claramente identificado como del contratante o asegurado (cliente). Si hay duda, devuelve null.
      ...
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'Eres un experto en análisis de documentos PDF de seguros. Extrae información específica de manera precisa y devuelve solo JSON válido. Para el campo email/e_mail: déjalo en blanco (null); nunca uses el correo de la compañía o del agente.'
            },
            {
              role: 'user',
              content: prompt
...
// Start server only when run directly (node server-mysql.js), not when required by Cloud Functions or Firebase CLI
const isMainModule = typeof require.main !== 'undefined' && require.main === module;
if (!isCloudFunctions && isMainModule) {
